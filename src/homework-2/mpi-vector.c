...
int main(void) {
  ...
  addition_vector = malloc(local_n * sizeof(double));
  ...
  Par_vector_addition(local_vec1, local_vec2, addition_vector, local_n);
  Print_vector(addition_vector, local_n, n, "addition vector of vector x and y:", my_rank, comm);
  
  free(addition_vector);
  free(local_vec1);
  free(local_vec2);
...
void Read_data(double *local_matrix, double *local_vector, int local_n, int my_rank, int comm_sz, MPI_Comm comm) {
  ...
  if (my_rank == 0) {
    ...
    // scatter vector 2
    MPI_Scatter(a, local_n, type, local_vector, local_n, type, 0, comm);
    free(a);
  } else {
    MPI_Scatter(a, local_n, type, local_matrix, local_n, type, 0, comm);
    MPI_Scatter(a, local_n, type, local_vector, local_n, type, 0, comm);
